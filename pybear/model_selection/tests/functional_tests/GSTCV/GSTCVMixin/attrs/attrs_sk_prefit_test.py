...
class TestAttrsPreFit:
    ...
    def test_attrs_1(self, refit, sk_GSTCV, generic_no_attribute, attr, _scoring):

        with pytest.raises(
            AttributeError,
            match=generic_no_attribute('GSTCV', attr)
        ):
            getattr(sk_GSTCV(refit=refit, scoring=_scoring), attr)
    ...
    @pytest.mark.parametrize('refit',(False, 'balanced_accuracy', lambda x: 0))
    def test_attrs_2(self, sk_GSTCV, refit, generic_no_attribute, generic_not_fitted,
        attr, _scoring
    ):
    ...
    @pytest.mark.parametrize('refit', (False, 'balanced_accuracy', lambda x: 0))
    def test_classes(self, sk_GSTCV, refit, generic_no_attribute, _scoring):
        ...
        if refit is False:
            ...
            with pytest.raises(AttributeError, match=exc_info('GSTCV')):
                sk_GSTCV(refit=refit, scoring=_scoring).classes_
    ...
    @pytest.mark.parametrize('refit', (False, 'balanced_accuracy', lambda x: 0))
    def test_n_features_in(self, sk_GSTCV, refit, generic_no_attribute, _scoring):

        exc_info = lambda x: f'{x} object has no n_features_in_ attribute.'

        with pytest.raises(AttributeError, match=exc_info("GSTCV")):
            sk_GSTCV(refit=refit, scoring=_scoring).n_features_in_

...
# @pytest.skip(reason=f'test takes 9 minutes', allow_module_level=True)
class TestCoreFitPipelineAccuracy:
    ...
    @staticmethod
    @pytest.fixture
    def good_estimator(good_est):
        return Pipeline(
            steps=[
                # ('DaskStandardScaler', dask_StandardScaler()),
                ('DaskLogistic', good_est)
            ]
        )
    ...
    @pytest.mark.parametrize('_param_grid',
        (
            [
                {# 'DaskStandardScaler__with_mean': [True, False],
                 'DaskLogistic__C': [.001, .01]}
            ],
            [
                {# 'DaskStandardScaler__with_mean': [True, False],
                 'DaskLogistic__C': [.001, .01]},

...
# @pytest.skip(reason=f'pizza', allow_module_level=True)
class TestCoreFitAccuracy:
    ...
    def good_estimator():
        return Pipeline(
            steps=[
                ...
                ('SKLogistic', dask_LogisticRegression(solver='lbfgs'))
            ]
        )
    ...
    def test_accuracy_cv_int_vs_cv_iter(self, good_X, good_y, good_estimator,
        ...
        good_cache_cv, good_iid, good_PARAM_GRID_KEY, good_THRESHOLD_DICT, _n_jobs):
    ...
    @pytest.mark.parametrize('_param_grid',
        (
            ...
            # ],
            # # # pizza, see notes somewhere around here about SK gridsearch anomalies
            # [
            #     {'OneHot__min_frequency': [None], 'SKLogistic__C': [.0001, .001]},

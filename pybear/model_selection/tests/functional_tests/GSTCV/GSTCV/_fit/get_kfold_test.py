...
class TestSKGetKFold:
    ...
    # X, y must both be np.ndarray

    # important!!! this function can be called multiple times within a
    # single param grid permutation, first to fit and get test score, then
    # again if return_train_score. Therefore, it must return the same
    # indices for each call. The only things that should cause indices
    # to be different are n_splits and the number of rows in X. For
    # StratifiedKFold, shuffle is always off.

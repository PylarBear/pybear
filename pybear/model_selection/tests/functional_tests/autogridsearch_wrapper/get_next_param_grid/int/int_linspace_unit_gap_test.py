...
class TestIntLinspaceUnitGap:
    ...
    def test_accuracy(self, _SINGLE_GRID, _posn, _is_hard, _hard_min,
                      _hard_max, _points):
        ...
        if _is_hard:
            ...
            if _posn == 'left':
                ...
                assert max(_grid_out) == min(_hard_max,
                             max(_hard_min, _SINGLE_GRID[0] - 1) + _points - 1)

            if _posn == 'right':
                assert min(_grid_out) == \
                    max(_hard_min,
                        min(_hard_max, _SINGLE_GRID[-1] + 1) - _points + 1
                )
                assert max(_grid_out) == min(_hard_max,
                             max(_SINGLE_GRID[-1] + 1, _hard_min + _points - 1)
                )

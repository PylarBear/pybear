...
from model_selection.GSTCV._cv_results_builder import _cv_results_builder
...
from model_selection.GSTCV._fit_shared._cv_results_update._cv_results_update \
...
class GSTCV(_GSTCVMixin):
    ...
    def fit(self, X: XInputType, y: YInputType=None, **params):
        ...
        **params: ???
        ...
        # BEFORE RUNNING cv_results_builder, THE THRESHOLDS MUST BE REMOVED
        # FROM EACH PARAM GRID IN wip_param_grids
        ...
        for trial_idx, _grid in enumerate(self.cv_results_['params']):
            ...
            # END create & verify holder objects ** * ** * ** * ** * ** * ** *

            ###################################################################
            # CORE GRID SEARCH ################################################

            KFOLD = list(_get_kfold(X, y, self.n_splits_, self.verbose, **kfold_params))
            ...
            with joblib.parallel_config(prefer='processes', n_jobs=self.n_jobs):
                FIT_OUTPUT = joblib.Parallel(return_as='list')(
                    joblib.delayed(_parallelized_fit)(
                        f_idx,
                        *_fold_splitter(train_idxs, test_idxs, X, y)[::2],  # train only!
                        train_idxs,
                        test_idxs,
                        *ARGS_FOR_PARALLEL_FIT,
                        **fit_params) for f_idx, (train_idxs, test_idxs) in enumerate(KFOLD)
                    )
            ...
            # PICK THE COLUMNS FROM TEST_FOLD_x_THRESHOLD_x_SCORER__SCORE_MATRIX THAT MATCH RESPECTIVE TEST_BEST_THRESHOLD_IDXS_BY_SCORER

...
from ._type_aliases import XType, YType

from ._validation._is_dask_estimator import _is_dask_estimator

from ._cv_results_builder.gstcv_cv_results_builder_2 import cv_results_builder

from pybear.base import is_classifier as pybear_is_classifier

from ._fit._parallelized_fit import _parallelized_fit
...
class GridSearchThresholdCV:
    ...
    def __init__(self,
        estimator,
        param_grid: Union[list[dict], dict],  # pizza
    ...
    def fit(self, X: XType, y: YType=None, **params):
        ...
        **params: ???
        ...
        if isinstance(X, (np.ndarray, da.core.Array, pd.DataFrame)):
            ...
            # AS OF 24_02_25_11_48_00 dask_Logistic CANNOT fit() ON A
            # dask.ddf (ONLY dask.array, pd.DF, np.array).
            # BUT UNEXPECTEDLY dask_Logistic WRAPPED BY dask_GridSearchCV
            # CAN fit() ON dask.ddfs.  THE ONLY WAY THIS COULD BE IS THAT
            # dask_GridSearchCV CONVERTS THE dask.ddf TO dask.array. IN
            # SKLEARN, WHEN pd.DF IS PASSED TO sklearn_GridSearchCV,
            # AFTER fit() "feature_names_in_" AND "n_features_in_" BECOME
            # AVAILABLE. "n_features_in_" IS NEVER MADE AVAILABLE BY
            # dask_GridSearchCV. "feature_names_in_" IS ALSO NEVER MADE
            # AVAILABLE BY dask_GridSearchCV, EVEN WHEN PASSED A pd.DF.
            # (REMEMBER dask_Logistic CAN fit() ON pd.DF, BUT NOT dask.ddf).
            # SO NO CURRENT FUNCTIONALITY IS LOST IF A dask.ddf IS
            # STRIPPED DOWN TO A dask.array.

            # IF IS SERIES, WONT HAVE columns (pizza??)
            try: self._feature_names_in = np.array(X.columns)
            except: pass

            X = X.to_dask_array(lengths=True)
        else:
            _exc(f"X passed with unknown data type '{type(X)}'. Use "
                 f"numpy.ndarray, dask.array.core.Array, pandas.Series, "
                 f"pandas.DataFrame, dask.dataframe.core.Series, "
                 f"dask.dataframe.core.DataFrame"
            )


        # CAPTURE THIS FOR DASK
        # DONT compute() HERE, JUST RETAIN & ONLY DO THE PROCESSING IF
        # n_features_in_ IS CALLED
        self._n_features_in_ = X.shape[1]
        # THIS IS A HOLDER THAT IS FILLED ONE TIME WHEN THE .compute() IS
        # DONE ON self._n_features_in_
        ...
        if isinstance(y, (np.ndarray, da.core.Array)):
            ...
            _exc(f"y passed with unknown data type '{type(y)}'. Use "
                f"numpy.ndarray, dask.array.core.Array, pandas.Series, "
                 f"pandas.DataFrame, dask.dataframe.core.Series, "
                 f"dask.dataframe.core.DataFrame"
            )
        ...
        def get_kfold(X: XType, y: YType=None, **kfold_params):
        ...
        def cv_results_score_updater(
                FOLD_x_SCORER__SCORES,
                _type,
                trial_idx
            ):
            ...
            for scorer_idx, scorer_suffix in enumerate(self.scorer_):
                ...
                for _split in range(self.n_splits_):
                    if f'split{_split}_{_type}_{scorer_suffix}' not in self.cv_results_:
                        no_column_msg(_type, 'split', _split, scorer_suffix)
                    self.cv_results_[f'split{_split}_{_type}_{scorer_suffix}'][trial_idx] = FOLD_x_SCORER__SCORES[_split, scorer_idx]

                if f'mean_{_type}_{scorer_suffix}' not in self.cv_results_:
                    no_column_msg(_type, 'mean', '', scorer_suffix)
                self.cv_results_[f'mean_{_type}_{scorer_suffix}'][trial_idx] = np.mean(FOLD_x_SCORER__SCORES[:, scorer_idx])

                if f'std_{_type}_{scorer_suffix}' not in self.cv_results_:
                    no_column_msg(_type, 'std', '', scorer_suffix)
        ...
        self.cv_results_, PARAM_GRID_KEY = \
            cv_results_builder(self.wip_param_grid, self.n_splits_, self.scorer_, self.return_train_score)
        ...
        for trial_idx, _grid in enumerate(self.cv_results_['params']):
            ...
            if self._dask_estimator:
                for f_idx, (train_idxs, test_idxs) in enumerate(CACHE_CV or get_kfold(X, y, **kfold_params)):
                    ...
                    del _
                    FIT_OUTPUT.append(_parallelized_fit(f_idx, X_train, y_train, train_idxs, test_idxs, *ARGS_FOR_PARALLEL_FIT, **fit_params))
                ...
                with joblib.parallel_config(backend='multiprocessing', n_jobs=self.n_jobs):
                    FIT_OUTPUT = \
                        joblib.Parallel(return_as='list')(
                            joblib.delayed(_parallelized_fit)(f_idx, *fold_splitter(train_idxs, test_idxs, X,y)[::2], train_idxs, test_idxs, *ARGS_FOR_PARALLEL_FIT, **fit_params) for
        ...
        del fold_splitter, threshold_scorer_sweeper, cv_results_score_updater

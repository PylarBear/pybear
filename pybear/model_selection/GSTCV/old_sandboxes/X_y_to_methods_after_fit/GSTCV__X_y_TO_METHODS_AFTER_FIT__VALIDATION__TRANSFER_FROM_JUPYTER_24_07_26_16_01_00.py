...
good_pd_X, good_pd_y = pd.DataFrame(good_np_X,
                                    columns=GOOD_COLUMNS), pd.DataFrame(
    good_np_y, columns=['y'])
good_ddf_X, good_ddf_y = ddf.from_pandas(good_pd_X,
                                         chunksize=_rows // 10), ddf.from_pandas(
    good_pd_y, chunksize=_rows // 10)

bad_np_X, bad_features_np_y = sklearn_make_classification(n_samples=_rows,
...
bad_pd_X, bad_features_pd_y = pd.DataFrame(bad_np_X,
                                           columns=BAD_COLUMNS), pd.DataFrame(
    bad_features_np_y, columns=['y1', 'y2'])
bad_ddf_X, bad_features_ddf_y = ddf.from_pandas(bad_pd_X,
                                                chunksize=_rows // 10), ddf.from_pandas(
    bad_features_pd_y, chunksize=_rows // 10)
...
COMBINATIONS = [f'{c}_{b}_{a}' for c in GOOD_OR_BAD_X for b in DTYPES for a in
                TYPES]
METHOD_ARRAY_DICT = {
    ...
    for _dtype in DTYPES:
        for _gscv_type in TYPES:
            ...
            try:
                ...
                METHOD_ARRAY_DICT = method_output_try_handler(trial,
                                                              'decision_function',
                                                              __,
                                                              METHOD_ARRAY_DICT)
            except:
                METHOD_ARRAY_DICT = method_output_except_handler(trial,
                                                                 'decision_function',
                                                                 sys.exc_info()[
                                                                     1],
                                                                 METHOD_ARRAY_DICT)
            ...
            try:
                ...
                METHOD_ARRAY_DICT = method_output_try_handler(trial, 'predict',
                                                              __,
                                                              METHOD_ARRAY_DICT)
            except:
                METHOD_ARRAY_DICT = method_output_except_handler(trial,

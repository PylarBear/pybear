...
def autogridsearch_docs():
    ...
    """
    Run multiple passes of GridSearchCV with progressively more precise
    ...
    space that minimize loss or maximize score for the particular estimator 
    and data set being fit. Sklearn / dask GridSearchCV.best_params_ is
    a dictionary with parameter names as keys and respective best values
    as values that is exposed by the fit() method of the parent GridSearch
    upon completion of a search over a set of grids. The fit() method of
    this wrapper class calls the parent GridSearch's fit() method to
    generate the best_params_ dictionary and uses the information provided
    in :param: params to calculate refined grids for the next search round.
    ...
    and all of the parent module's methods and attributes are accessible
    via the AutoGridSearch instance. This, however, precludes AutoGridSearch
    from using the same API itself (doing so would overwrite the
    underlying's.) So methods like set_params, get_params, etc., access
    the parent GridSearchCV and not the child AutoGridSearch wrapper. The
    ...
    attribute is a dictionary of all the best values returned by the
    parent GridSearch during the AutoGridSearch session. It is a
    ...
    will terminate if instructions are passed to the params argument
    that violate the universal bounds. There is no logical upper bound
    for integers or floats. Negative search spaces are not possible under
    any circumstance in AutoGridSearch.
    ...
    at the start is the only search grid for every pass with one exception:
    the search space can only be changed by specifying a pass (and the
    passes thereafter) on which to shrink the space to a single value
    ...
    integer search grid, breaking the universal minimum bound for integers,
    whereas all other integer search spaces observe the universal lower
    bound of 1.
    ...
    grid = [20, 21, 22, 23, 24]. If the best value returned by GridSearchCV
    is 20, then a 'left-shift' is affected by decrementing every value
    in the grid by max(grid) - grid[1] -> 3. The search grid for the next
    round is [17, 18, 19, 20, 21]. Similarly, if the best value returned
    by GridSearchCV is 24, then a 'right-shift' is affected by incrementing
    every value in the grid by grid[-2] - min(grid) -> 3. The search grid
    for the next round is [23, 24, 25, 26, 27]. String, bool, fixed, and
    hard spaces are not shifted. If passed any 'soft' spaces,
    AutoGridSearch will perform shifting passes until 1) it reaches a
    pass in which all soft parameters' best values simultaneously fall
    off the edges of their search grids, 2) 'max_shifts' is reached, or
    3) total_passes_is_hard is True and total_passes is reached.
    ...
    that search. Not applicable to 'fixed', 'bool', or 'string' parameters.
    Briefly and simplistically, the next search grid is a 'zoom-in' on
    the last round's (sorted) grid in the region created by the search
    values that are adjacent to the best value. For float search spaces,
    all intervals are infinitely divisible and will be divided according
    to the number of points provided in 'params'. For integer search
    spaces, when the limit of unit intervals is approached, the search
    space is divided with unit intervals and the number of points to
    search is adjusted accordingly, regardless of the number of search
    points stated in params, and params is overwritten with the new number
    of points.
    ...
    is designed to avoid this condition as much as possible by handling
    all large-scale shifting first.
    ...
    entered as pass 3, that is incremented to pass 4. For other parameters
    that use 'points', all the points in their lists are moved out by one
    round, also moving out any shrink pass(es) included in their original
    instructions.
    ...
    total_passes for each shifting pass. For example, consider a situation
    where we set total_passes to 3 and we know beforehand that
    ...
    for search over astronomically large spaces. Once shifting requirements
    for any parameter are fulfilled (best value is framed or max shifts
    is reached), if that parameter has a log10 interval greater than 1
    AutoGridSearch regaps that logspace to unit interval. This is to allow
    sufficient fidelity in the search grid for other parameters to be
    able to surround their true global optima, or at least get closer.
    ...
    Restrictions
    ------------
    Integer search spaces must be greater than or equal to 1. Float search
    spaces must be greater than or equal to 0. 'Soft' search grids must
    have at least 3 points. Logarithmic search intervals must be base 10
    and the first grid must contain integers, even for a 'float' space.
    Booleans cannot be passed to an 'integer' or 'float' space. Integers
    and floats cannot be passed to a boolean space.
    
    Params Argument
    ---------------
    'params' must be a single dictionary. AutoGridSearch cannot accommodate
    multiple params entries in the same way that sci-kit learn and dask
    GridSearchCV can accomodate multiple param_grids.
    ...
    ** * ** * **
    ...
    all grid searches for this parameter, with the one exception of shrink.
    ...
    ** * ** * **
    ...
    all types, as it informs AutoGridSearch how to handle the instructions
    and grids. There are six allowed entries for numerical parameters:
    ...
    ** * ** * **
    ...
    ----------
    estimator:
        any estimator that follows the scikit-learn fit / score / get_params
        API. Includes scikit-learn, dask, lightGBM, and xgboost estimators.
    params:
    ...
    ... )
    >>> params = {
    ...     'C': [[5_000, 10_000, 15_000], [3, 3, 3], 'soft_Float'],

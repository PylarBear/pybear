...
def _int_logspace_gap_gt_1_soft(
    ...
    based on whether the best result from GridSearchCV landed on a left
    or right edge or in the middle of last round's search grid. Apply
    the universal integer lower bound of 1 (10**0) to the left bound.
    Interstitial values are determined by another module.

    Parameters
    ----------
    _LOG_SINGLE_GRID:
        ndarray[int] - The last round's search grid for a single soft
        integer parameter in logspace. _LOG_SINGLE_GRID must be sorted
    ...
    if not isinstance(_LOG_SINGLE_GRID, np.ndarray):
        raise TypeError(f"_LOG_SINGLE_GRID must be an ndarray (_SINGLE_GRID "

...
# 3/26/23 TEST sd.core_insert_outer, sd.insert_outer, sd.append_outer FOR FUNCTIONALITY

# CAN ONLY PROVIDE DICT AS INPUT, INSERT OBJECTS CAN BE NP OR SD


import pytest
pytest.skip(reason=f'24_09_07_09_18_00 needs work', allow_module_level=True)
...
########################################################################################################################
# TEST core_insert_outer ###############################################################################################
print(f'\n\033[92m*** STARTING core_insert_outer() TEST ***\033[0m')
            ...
            for ins_orientation in MASTER_INSERT_ORIENTATION:
                for single_double in MASTER_INSERT_SINGLE_DOUBLE:
                    ...
                    oper_desc = f'\033[92mInsert a {single_double}{" ( [[]] )" if single_double=="DOUBLE" else " ( [] )" if single_double=="SINGLE" else ""} ' \
                                f'{ins_format} of 5 rows by {ins_size} columns oriented as {ins_orientation} into the {ins_idx} outer ' \
                    ...
                    EXP_DICT = np.insert(EXP_DICT, ins_idx, DUM_INSERT, axis=0); del DUM_INSERT
                    EXP_DICT = sd.zip_list_as_py_int(EXP_DICT)
                    # END BUILD EXP_DICT #################################################################################

                    # HEADER IS NOT A FEATURE OF core_insert_outer
                    ACT_DICT = sd.core_insert_outer(GIVEN_INPUT_DICT, ins_idx, GIVEN_INS_OBJ)

                    if not sd.core_sparse_equiv(EXP_DICT, ACT_DICT):
                        test_fail('core_insert_outer', oper_desc, GIVEN_INPUT_DICT, EXP_DICT, ACT_DICT)

print(f'\n\033[92m*** core_insert_outer TESTS COMPLETE. ALL TESTS PASSED. ***\033[0m')
# END TEST core_insert_outer ###########################################################################################
########################################################################################################################
...
########################################################################################################################
# TEST insert_outer ####################################################################################################
print(f'\n\033[92m*** STARTING insert_outer() TEST ***\033[0m')
                            ...
                            for ins_orientation in MASTER_INSERT_ORIENTATION:
                                for obj_single_double in MASTER_INSERT_SINGLE_DOUBLE:
                                    ...
                                    oper_desc = f'\033[92mInsert a {obj_single_double}{" ( [[]] )" if obj_single_double == "DOUBLE" else " ( [] )" if obj_single_double == "SINGLE" else ""} ' \
                                                ...
                                                f'{"" if obj_header_desc is None else header_axis} axis (insertion only happens along the 0 axis). ' \
                                    ...
                                    EXP_DICT = np.insert(EXP_DICT, ins_idx, DUM_INSERT, axis=0)
                                    del DUM_INSERT
                                    EXP_DICT = sd.zip_list_as_py_int(EXP_DICT)
                                    # END BUILD EXP_DICT #################################################################################

                                    # BUILD EXP HEADER ###################################################################################
                                    # IF (INS AND DICT1 HEADERS GIVEN, OR ONE OR THE OTHER GIVEN) THEN A HEADER SHOULD BE RETURNED BY insert_outer OTHERWISE RETURNS None
                                    if obj_header_desc is None and ins_header_desc is None:
                                        ...
                                        # ALL INSERTIONS ONLY CHANGE AXIS 0. SO IF HEADER IS AXIS 1 THERE IS NO CHANGE TO GIVEN_OBJ_HEADER
                                        if header_axis==1:
                                    ...
                                    if ins_header_desc is None and obj_header_desc is None:
                                        ...
                                        ACT_DICT = sd.insert_outer(GIVEN_INPUT_DICT, ins_idx, GIVEN_INS_OBJ,
                                           DICT_HEADER1=GIVEN_OBJ_HEADER, INSERT_HEADER=GIVEN_INS_HEADER, header_axis=header_axis, fxn='insert_outer_test')

                                        if not sd.core_sparse_equiv(EXP_DICT, ACT_DICT):
                                            test_fail('insert_outer', oper_desc, GIVEN_INPUT_DICT, EXP_DICT, ACT_DICT)

                                    else: # elif not ins_header_desc is None and not obj_header_desc is None:
                                        ACT_DICT, ACT_DICT_HEADER = sd.insert_outer(GIVEN_INPUT_DICT, ins_idx, GIVEN_INS_OBJ,
                                            DICT_HEADER1=GIVEN_OBJ_HEADER, INSERT_HEADER=GIVEN_INS_HEADER, header_axis=header_axis, fxn='insert_outer_test')
                                        ...
                                        if not np.array_equiv(EXP_HEADER, ACT_DICT_HEADER):
                                            test_fail('insert_outer', oper_desc, GIVEN_OBJ_HEADER, EXP_HEADER, ACT_DICT_HEADER)

print(f'\n\033[92m*** insert_outer() TESTS COMPLETE. ALL TESTS PASSED. ***\033[0m')
# END TEST insert_outer ################################################################################################
########################################################################################################################
...
########################################################################################################################
# TEST append_outer ####################################################################################################
print(f'\n\033[92m*** STARTING append_outer() TEST ***\033[0m')
                    ...
                    for ins_format in MASTER_INSERT_FORMAT:
                        for ins_orientation in MASTER_INSERT_ORIENTATION:
                            for obj_single_double in MASTER_INSERT_SINGLE_DOUBLE:
                                ...
                                test_name = 'append_outer'

                                if ins_size == 2: obj_single_double = 'DOUBLE'
                                if ins_orientation == 'ROW': obj_single_double = 'DOUBLE'

                                oper_desc = f'\033[92mAppend a {obj_single_double}{" ( [[]] )" if obj_single_double == "DOUBLE" else " ( [] )" if obj_single_double == "SINGLE" else ""} ' \
                                            f'{ins_format} of 5 rows by {ins_size} columns oriented as {ins_orientation} into the last outer ' \
                                            f'index position of a 5 x 5 SPARSE_DICT.' \
                                            f'\nINPUT DICT header is{" not" if obj_header_desc is None else " "}given{"." if obj_header_desc is None else " along the "}' \
                                            f'{"" if obj_header_desc is None else header_axis} axis (insertion only happens along the 0 axis). ' \
                                            f'INSERT header is{" not" if ins_header_desc is None else " "}given.\033[0m\n'
                                ...
                                EXP_DICT = np.insert(EXP_DICT, len(EXP_DICT), DUM_INSERT, axis=0)
                                del DUM_INSERT
                                EXP_DICT = sd.zip_list_as_py_int(EXP_DICT)
                                # END BUILD EXP_DICT #################################################################################
                                ...
                                if ins_header_desc is None and obj_header_desc is None:
                                    ...
                                    ACT_DICT = sd.append_outer(GIVEN_INPUT_DICT,
                                                               GIVEN_INS_OBJ,
                                                               DICT_HEADER1=GIVEN_OBJ_HEADER,
                                                               INSERT_HEADER=GIVEN_INS_HEADER,
                                                               header_axis=header_axis,
                                                               fxn='append_outer_test')

                                    if not sd.core_sparse_equiv(EXP_DICT, ACT_DICT):
                                        test_fail('append_outer', oper_desc, GIVEN_INPUT_DICT, EXP_DICT, ACT_DICT)

                                else:  # elif not ins_header_desc is None and not obj_header_desc is None:
                                    ACT_DICT, ACT_DICT_HEADER = sd.append_outer(GIVEN_INPUT_DICT,
                                                                                GIVEN_INS_OBJ,
                                                                                DICT_HEADER1=GIVEN_OBJ_HEADER,
                                                                                INSERT_HEADER=GIVEN_INS_HEADER,
                                                                                header_axis=header_axis,
                                    ...
                                    if not np.array_equiv(EXP_HEADER, ACT_DICT_HEADER):
                                        test_fail('append_outer', oper_desc, GIVEN_OBJ_HEADER, EXP_HEADER,
                                                  ACT_DICT_HEADER)

print(f'\n\033[92m*** append_outer() TESTS COMPLETE. ALL TESTS PASSED. ***\033[0m')
# END TEST append_outer ################################################################################################
########################################################################################################################

...
class TestSymmetricMatmulCheck:
    ...
    def test_rejects_non_dict(self, non_dict, valid_dict1, valid_dict2):
        with pytest.raises(TypeError):
            val._symmetric_matmul_check(non_dict, valid_dict2)
            val._symmetric_matmul_check(valid_dict1, non_dict)
            val._symmetric_matmul_check(non_dict, non_dict)


    def test_correctly_returns_true(self, valid_dict1, valid_dict1_t,
                                    valid_dict2, valid_dict2_t):
        assert val._symmetric_matmul_check(valid_dict1, valid_dict1_t)
        assert val._symmetric_matmul_check(valid_dict2, valid_dict2_t)


    def test_correctly_returns_false(self, valid_dict1, valid_dict2):
        assert not val._symmetric_matmul_check(valid_dict2, valid_dict2)
        assert not val._symmetric_matmul_check(valid_dict1, valid_dict2)
    ...
    def test_rejects_bad_dict1_transpose(self, valid_dict1, valid_dict2,
                                         bad_dict1_t):
        with pytest.raises(ValueError):
            val._symmetric_matmul_check(valid_dict1, valid_dict2,

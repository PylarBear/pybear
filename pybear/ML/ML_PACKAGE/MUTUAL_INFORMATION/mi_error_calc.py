...
def mi_error_calc(OUTPUT_VECTOR, TARGET_VECTOR):
    ...
    if output_format=='SPARSE_DICT': OUTPUT_VECTOR = sd.unzip_to_ndarray_float64(OUTPUT_VECTOR)[0].reshape((1,-1))[0]

    if target_format=='SPARSE_DICT': TARGET_VECTOR = sd.unzip_to_ndarray_float64(TARGET_VECTOR)[0].reshape((1,-1))[0]

    new_error_start = np.sum((TARGET_VECTOR.reshape((1,-1))[0] - OUTPUT_VECTOR.reshape((1,-1))[0])**2)
...
if __name__ == '__main__':
    ...
    total_trials = np.product(list(map(len,
                       (MASTER_OUTPUT_FORMAT, MASTER_OUTPUT_ORIENT, MASTER_TARGET_FORMAT, MASTER_TARGET_ORIENT))))
        ...
        for output_orient in MASTER_OUTPUT_ORIENT:
            for target_format in MASTER_TARGET_FORMAT:
                for target_orient in MASTER_TARGET_ORIENT:
                    ...
                    act_error = mi_error_calc(GIVEN_OUTPUT, GIVEN_TARGET)

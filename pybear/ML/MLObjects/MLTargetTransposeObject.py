...
# 12/16/22 THIS MODULE HANDLES "TARGET_TRANSPOSE" AS OPPOSED TO "TARGET".  THE TWO CLASSES EXIST TO MANAGE THE VICE-VERSA
# TYPE RELATIONSHIP OF VALIDATION FOR IS/ISNT MULTI-CLASS


class MLTargetTransposeObject(mlo.MLObject):
    '''Child class for handling TARGET_TRANSPOSE within MLReqression operations. May be multi-class. Object is accessed as attribute of class.'''
    ...
    def __init__(self, OBJECT, given_orientation, return_orientation='AS_GIVEN', return_format='AS_GIVEN',
        ...
        super().__init__(OBJECT, given_orientation, name='TARGET_TRANSPOSE', return_orientation=return_orientation,
                return_format=return_format, bypass_validation=bypass_validation, calling_module=calling_module,
    ...
    def object_init(self):
        ...
        if self.is_multiclass:
            error_text = f'Given TARGET TRANSPOSE OBJECT appears to be single class but is declared as multi-class'
            if self.given_orientation == 'COLUMN' and self.inner_len == 1: is_error = True
            elif self.given_orientation == 'ROW' and self.outer_len == 1: is_error = True
        elif not self.is_multiclass:
            error_text = f'Given TARGET TRANSPOSE OBJECT appears to be multi-class but is declared as single class'
            if self.given_orientation == 'ROW' and self.outer_len > 1: is_error = True
            elif self.given_orientation == 'COLUMN' and self.inner_len > 1: is_error = True
...
if __name__ == '__main__':
    ...
    exp_calling_module = 'MLTargetTransposeObject'
                ...
                for exp_return_orientation in ['COLUMN', 'ROW']:
                    ...
                    for exp_return_format in ['ARRAY', 'SPARSE_DICT']:
                        ...
                        for exp_bypass_validation in [True, False]:
                            ...
                            DummyObject = MLTargetTransposeObject(GIVEN_OBJECT,

...
def excel_csv_pandas_reader(filename, filetype, SHEET, header, use_cols_select, number_of_rows, suppress_print='N'):

    if filetype.upper() == 'EXCEL':
        EXCEL_CSV_DF = p.DataFrame(
                                p.read_excel(filename, header=header, sheet_name=SHEET,
        ...
        EXCEL_CSV_DF = p.DataFrame(
                                p.read_csv(filename, header=header,
                                index_col=None, nrows=number_of_rows,
                                usecols=use_cols_select)
        )
    ...
    if 'LIST' in str(type(header)).upper():  # IF header IS LIST THEN LEN(header) IS > 1, OTHERWISE IT IS A SINGLE INT
        if vui.validate_user_str(f'Merge header rows? (y/n) > ', 'YN') == 'Y':
            ...
            # BEAR 24_04_09_16_01_00 THIS WAS ONCE ITS OWN MODULE BUT WAS ONLY CALLED HERE
...
if __name__ == '__main__':
    ...
    excel_csv_pandas_reader(filename, filetype, SHEET, header, use_cols_select, number_of_rows, suppress_print='N')
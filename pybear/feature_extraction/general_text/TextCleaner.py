...
from general_text import alphanumeric_str as ans, stop_words as sw, statistics as stats
...
class TextCleaner:
    ...
    def __init__(self, LIST_OF_STRINGS, update_lexicon=True, auto_add=False, auto_delete=False):  # return_as_list_of_lists=False,

        # LIST_OF_STRINGS MUST BE PASSED AS ['str1','str2','str3'...], JUST LIKE NNLM50

        # auto_add AUTOMATICALLY ADDS AN UNKNOWN WORD TO LEXICON_UPDATE W/O PROMPTING USER
        # (JUST GOES ALL THE WAY THRU WITHOUT PROMPTS) AUTOMATICALLY SENSES AND MAKES 2-WAY SPLITS
        ...
        _dtype, LIST_OF_STRINGS = ldv.list_dict_validater(LIST_OF_STRINGS, 'LIST_OF_STRINGS')  # RETURNS AS [[]]

        if _dtype != 'ARRAY':
            self._exception(f'LIST_OF_STRINGS MUST BE A LIST-TYPE THAT CAN BE CONVERTED TO NUMPY ARRAY', fxn)
        del _dtype

        try: self.LIST_OF_STRINGS = np.fromiter(map(str, LIST_OF_STRINGS[0]), dtype='<U10000')
        except: self._exception(f"LIST_OF_STRINGS MUST CONTAIN DATA THAT CAN BE CONVERTED TO str", fxn)

        self.CLEANED_TEXT = self.LIST_OF_STRINGS.copy().astype(object)
                # np.empty((1, len(self.LIST_OF_STRINGS)), dtype=object)[0]

        self.update_lexicon = akv.arg_kwarg_validater(update_lexicon, 'update_lexicon', [True, False, None],
                                          self.this_module, fxn, return_if_none=False)

        self.auto_add = akv.arg_kwarg_validater(auto_add, 'auto_add', [True, False, None],
                                          self.this_module, fxn, return_if_none=False)

        self.auto_delete = akv.arg_kwarg_validater(auto_delete, 'auto_delete', [True, False, None],
                                          self.this_module, fxn, return_if_none=False)

        # if self.update_lexicon is True and self.auto_delete is True:
            # 2/7/23 auto_delete CAUSES BYPASS OF HANDLING OPTIONS FOR UNKNOWN WORDS DURING lex_lookup PROCESSES.  CANNOT
            # update_lexicon IF BEING BYPASSED. update_lexicon ENABLES OR DISABLES THE add to lexicon MENU OPTION DURING
        ...
        self.LEX_LOOK_DICT = {'A': 'ADD TO LEXICON', 'D': 'DELETE', 'E': 'EDIT', 'F': 'EDIT ALL', 'L': 'DELETE ALL',
        'S': 'SPLIT', 'U': 'SPLIT ALWAYS', 'K': 'SKIP ONCE', 'W': 'SKIP ALWAYS', 'Y': 'VIEW LEXICON ADDENDUM',
        'Z': 'GO TO MAIN MENU'}  # DONT USE 'T' !!!
        if not self.update_lexicon: del self.LEX_LOOK_DICT['A']
    ...
    def menu(self, allowed=None, disallowed=None):
        ...
        if not allowed is None and not disallowed is None:
            ...
            if not isinstance(allowed, str): self._exception(f'{fxn} allowed KWARG REQUIRES str AS INPUT', fxn)
            for _char in allowed:
                if not _char in allowed_key: self._exception(f'INVALID KEY "{_char}" IN allowed, MUST BE IN {allowed_key}.', fxn)
        elif not disallowed is None:
            if not isinstance(disallowed, str): self._exception(f'{fxn} disallowed KWARG REQUIRES str AS INPUT', fxn)
            for _char in disallowed:
                if not _char in allowed_key: self._exception(f'INVALID KEY "{_char}" IN disallowed, MUST BE IN {allowed_key}.', fxn)
        ...
        [_ if _ in allowed_key else self._exception(
            f'{fxn} allowed KWARG CHARACTERS MUST BE IN *{allowed_key}*', fxn) for _ in allowed ]
        # END VALIDATION ################################################################################################
        ...
        while True:
            ...
            # MUST UPDATE "UNDO TOGGLE" 'label' VALUE EVERY TIME EXPLICITLY, OTHERWISE IT IS NOT UPDATING.  IT APPEARS THAT
            # THE VALUE IS BEING SET THE ONE TIME DURING init, BUT ONCE WRIT THE VALUE IS HARD-CODED AND NO LONGER SEES
            # A {} THERE.
            ...
            # ONLY CREATE A NEW BACKUP COPY WHEN GOING TO delete_empty_rows, remove_characters, strip, normalize,
            ...
            elif selection == 'Q': self._exception(f'USER TERMINATED', fxn)
            elif selection == 'Z': del display; return self.CLEANED_TEXT   # 4-8-23 ALLOW RETURN OF CLEANED_TEXT FROM menu()
    ...
    def remove_characters(self, allowed_chars_as_str=ans.alphanumeric_str(), disallowed_chars_as_str=None):
        ...
        if not self.is_list_of_lists:   # MUST BE LIST OF strs
            ...
            for row_idx in range(len(self.CLEANED_TEXT)):
                ...
                if not f'' in self.CLEANED_TEXT[row_idx]: continue
                else: self.CLEANED_TEXT[row_idx] = self.CLEANED_TEXT[row_idx][..., self.CLEANED_TEXT[row_idx]!='']
        ...
        for row_idx in range(len(self.CLEANED_TEXT)):
            if self.is_list_of_lists:
                while True:
                    ...
                    else: map(str.strip, self.CLEANED_TEXT[row_idx]); break
                ...
                while True:
                    ...
                    else: self.CLEANED_TEXT[row_idx] = self.CLEANED_TEXT[row_idx].strip(); break
    ...
    def normalize(self, upper=True):    # IF NOT upper THEN lower
        ...
        if self.is_list_of_lists:
            for row_idx in range(len(self.CLEANED_TEXT)):
                if upper: self.CLEANED_TEXT[row_idx] = np.fromiter(map(str.upper, self.CLEANED_TEXT[row_idx]), dtype='U30')
                elif not upper: self.CLEANED_TEXT[row_idx] = np.fromiter(map(str.lower, self.CLEANED_TEXT[row_idx]), dtype='U30')
        elif not self.is_list_of_lists:   # LIST OF strs
            if upper: self.CLEANED_TEXT = np.fromiter(map(str.upper, self.CLEANED_TEXT), dtype='U100000')
            elif not upper: self.CLEANED_TEXT = np.fromiter(map(str.lower, self.CLEANED_TEXT), dtype='U100000')


    def return_row_uniques(self, return_counts=False):
        '''Return a potentially ragged vector containing the unique words for each row in CLEANED_TEXT object.'''
        ...
        if not return_counts:
            ...
            for row_idx in range(len(self.CLEANED_TEXT)):
                UNIQUES_HOLDER[row_idx], COUNTS_HOLDER[row_idx] = np.unique(self.CLEANED_TEXT[row_idx], return_counts=True)
    ...
    def view_row_uniques(self, return_counts=None):
        ...
        return_counts = akv.arg_kwarg_validater(return_counts, 'return_counts', [True, False, None], self.this_module, fxn)

        if return_counts is None: return_counts = {'Y':True, 'N':False}[vui.validate_user_str(f'View counts? (y/n) > ', 'YN')]

        if return_counts is True: UNIQUES, COUNTS = self.return_row_uniques(return_counts=True)
        elif return_counts is False: UNIQUES = self.return_row_uniques(return_counts=False)
        ...
        if return_counts is True: del UNIQUES, COUNTS
        elif return_counts is False:  del UNIQUES
    ...
    def view_overall_uniques(self, return_counts=None):
    ...
    def remove_stops(self):
        ...
        for row_idx in range(len(self.CLEANED_TEXT)):
            for word_idx in range(len(self.CLEANED_TEXT[row_idx])-1, -1, -1):
                if self.CLEANED_TEXT[row_idx][word_idx] in sw.stop_words():
                    self.CLEANED_TEXT[row_idx] = np.delete(self.CLEANED_TEXT[row_idx], word_idx, axis=0)
                if len(self.CLEANED_TEXT) == 0: break

        if converted: self.as_list_of_strs()
        del converted


    def justify(self, chars=None):
        '''Fit text as strings or as lists to user-specified number of characters per row.'''

        # ALSO SEE general_text.notepad_justifier FOR SIMILAR CODE, IF EVER CONSOLIDATING

        fxn = inspect.stack()[0][3]

        if not chars is None:
            chars = akv.arg_kwarg_validater(chars, 'characters', list(range(30,50001)), self.this_module, fxn)
        elif chars is None:  # DONT PUT THIS IN akv(return_if_none=)... PROMPTS USER FOR INPUT BEFORE ENDING args/kwargs TO akv
            chars = vui.validate_user_int(f'\nEnter number of characters per line (min=30, max=50000) > ', min=30, max=50000)
        ...
        self.CLEANED_TEXT = NEW_TXT; del NEW_TXT
        self.is_list_of_lists = False
    ...
    def delete_words(self):
        ...
        while True:
            ...
            if to_delete in 'Z': del ctr; break
            print(f'\nUser entered to delete')
            print(", ".join(TO_DELETE))
            if vui.validate_user_str(f'\nAccept? (y/n) > ', 'YN') == 'Y': del ctr; break
            else: TO_DELETE = np.empty((1,0), dtype='<U30')[0]; ctr=0
    ...
    def substitute_words(self):
        ...
        while True:
            ...
            while True:
                ...
                if vui.validate_user_str(f'User entered to replace *{replaced}* with *{replacement}*--- Accept? (y/n) > ',
                        'YN') == 'Y':
                    ctr += 1
    ...
    def as_list_of_strs(self):
        ...
        if not self.is_list_of_lists: pass
        elif self.is_list_of_lists:
    ...
    def statistics(self):
        ...
        stats.statistics(np.hstack(self.CLEANED_TEXT))
    ...
    def word_counter(self):
        ...
        is_asc = True
        SUB_MENU = {'e': 'exit', 'c': 'set count cutoff', 'd': 'dump to file', 'p': 'print table to screen',
                    'r': 'print chart to screen', 's': f'change sort (currently XXX)'}  # DONT FORGET 's' BELOW
        cutoff_ct = 100 # SEED
        ...
        while True:
            ...
            if selection == 'e': #'exit'
                ...
                _core_fxn = pd.DataFrame(data=np.vstack((WIP_UNIQUES, WIP_COUNTS)).transpose(), columns=[f'WORD', f'COUNT']).to_csv
                ...
                fig_widget.write_html('word_frequency.html', auto_open=True)    # MUST END IN .html SO IT KNOWS TO OPEN IN BROWSER
                del fig, fig_widget
            elif selection == 's': #'change sort (currently XXX)'
                if is_asc: is_asc = False
                elif not is_asc: is_asc = True
    ...
    def dump_to_file_wrapper(self, core_write_function, _ext, kwargs):
        ...
        while True:
            ...
            if __ == 'Y':
                core_write_function(base_path+file_name+_ext, **kwargs)
    ...
    def lex_lookup_menu(self, allowed=None, disallowed=None, fxn=None):    # ALLOWED ARE 'adelks'
        ...
        if not allowed is None and not disallowed is None:
            ...
            if not isinstance(allowed, str): self._exception(fxn, f'{inspect.stack()[0][3]}() allowed kwarg must be a single string')
            ...
            if not isinstance(disallowed, str): self._exception(fxn, f'{inspect.stack()[0][3]}() disallowed kwarg must be a single string')
    ...
    def word_editor(self, word, prompt=None):
        ...
        while True:
            ...
            if vui.validate_user_str(f'USER ENTERED *{word}* -- ACCEPT? (Y/N) > ', 'YN') == 'Y': break
    ...
    def lex_lookup_add(self, word):
        ...
        self.LEXICON_ADDENDUM = np.insert(self.LEXICON_ADDENDUM, len(self.LEXICON_ADDENDUM), word, axis=0)
        self.KNOWN_WORDS = np.insert(self.KNOWN_WORDS, len(self.KNOWN_WORDS), word, axis=0)
    ...
    def lex_lookup(self, print_notes=False):
        '''Scan entire CLEANED_TEXT object and prompt for handling of words not in LEXICON.'''
        ...
        print_notes = akv.arg_kwarg_validater(print_notes, 'print_notes', [True, False, None], self.this_module,
        ...
        if self.update_lexicon:
            ...
            if len(self.LEXICON_ADDENDUM) != 0:
                ...
                if _ == 'A': _abort = True
                elif _ == 'E': self.LEXICON_ADDENDUM = np.empty((1,0), dtype='<U30')[0]
                del _
        ...
        for row_idx in [range(len(self.CLEANED_TEXT)) if not _abort else []][0]:
            ...
            if number_of_edits in range(10,int(1e6),10):
                if vui.validate_user_str(f'Save all that hard work to file(s) or continue(c) > ', 'SC') == 'S':
                    if vui.validate_user_str(f'Save to csv(c) or txt(t)? > ', 'CT') == 'C': self.dump_to_csv()
                    else: self.dump_to_txt()
                else: number_of_edits += 1   # SO IT DOESNT GET HUNG UP IF number_of_edits DOESNT CHANGE AS GOING THRU ROWS
            ...
            for word_idx in range(len(self.CLEANED_TEXT[row_idx])-1, -1, -1):
                ...
                if self.auto_delete:
                ...
                elif word in self.KNOWN_WORDS:    # IMPLICIT not self.auto_delete
                    if print_notes: print(f'\n*** {word} IS ALREADY IN LEXICON ***\n')
                elif word in WORDS_TO_DELETE:     # IMPLICIT not self.auto_delete
                    if print_notes: print(f'\n*** DELETING {word} ***\n')
                    self.CLEANED_TEXT[row_idx] = np.delete(self.CLEANED_TEXT[row_idx], word_idx, axis=0)
                elif word in SKIP_ALWAYS:         # IMPLICIT not self.auto_delete
                    continue
                elif word in EDIT_ALL_DICT:       # IMPLICIT not self.auto_delete
                    self.CLEANED_TEXT[row_idx][word_idx] = EDIT_ALL_DICT[word]
                elif word in SPLIT_ALWAYS_DICT:   # IMPLICIT not self.auto_delete
                    self.CLEANED_TEXT[row_idx] = np.delete(self.CLEANED_TEXT[row_idx], word_idx, axis=0)
                    for slot_idx in range(len(SPLIT_ALWAYS_DICT[word]) - 1, -1, -1):  # GO THRU NEW_WORDS BACKWARDS
                    ...
                    if _ == 'A': self.lex_lookup_add(word)
                    elif _ == 'D':
                        self.CLEANED_TEXT[row_idx] = np.delete(self.CLEANED_TEXT[row_idx], word_idx, axis=0)
                    elif _ in 'EF':
                        ...
                        if _ == 'F': EDIT_ALL_DICT[word] = new_word
                        ...
                        if self.update_lexicon:
                            if new_word not in self.KNOWN_WORDS and new_word not in SKIP_ALWAYS:
                                ...
                                if __ == 'A': self.lex_lookup_add(new_word)
                                elif __ == 'K': pass
                                elif __ == 'W': SKIP_ALWAYS = np.insert(SKIP_ALWAYS, len(SKIP_ALWAYS), new_word, axis=0)
                    ...
                    elif _ == 'W': SKIP_ALWAYS = np.insert(SKIP_ALWAYS, len(SKIP_ALWAYS), word, axis=0)
                    elif _ in 'SU':
                        while True:
                            ...
                            if vui.validate_user_str(f'User entered *{", ".join(NEW_WORDS)}* > accept? (y/n) > ', 'YN') == 'Y':
                                if _ == 'U': SPLIT_ALWAYS_DICT[word] = NEW_WORDS
                        ...
                        if self.update_lexicon:
                            ...
                            for slot_idx in range(len(NEW_WORDS)):
                                if NEW_WORDS[slot_idx] not in self.KNOWN_WORDS and NEW_WORDS[slot_idx] not in SKIP_ALWAYS:
                                    ...
                                    if _ == 'A': self.lex_lookup_add(NEW_WORDS[slot_idx])
                                    elif _ == 'K': pass
                                    elif _ == 'W': SKIP_ALWAYS = np.insert(SKIP_ALWAYS, len(SKIP_ALWAYS), word, axis=0)
                            del SUB_MENU, sub_allowed
                        del NEW_WORDS
        ...
        del MENU_DISPLAY, menu_allowed, number_of_edits, word_counter, WORDS_TO_DELETE, SKIP_ALWAYS, EDIT_ALL_DICT, \
            SPLIT_ALWAYS_DICT
        self.KNOWN_WORDS = None
...
if __name__ == '__main__':
    ...
    # ALSO SEE general_text.misc_text.TextCleaner__test_module

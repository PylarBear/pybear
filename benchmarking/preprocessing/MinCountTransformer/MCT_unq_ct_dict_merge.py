...
# do two consecutive partial fits on the same data, _tcbc should show
# double the counts of one pass

_mct.partial_fit(X, y=y)
print()
print(f'CT_DICT_1:')
__ = deepcopy(_mct._total_counts_by_column)
for _c_idx in __:
    __[_c_idx] = dict((
        zip(
            list(map(float, __[_c_idx].keys())),
            list(map(int, __[_c_idx].values()))
        )
    ))
    print(f"{_c_idx}: {__[_c_idx]}")
print()

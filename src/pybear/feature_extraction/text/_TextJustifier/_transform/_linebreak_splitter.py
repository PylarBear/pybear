...
def _linebreak_splitter(
    _X: list[str],
    _line_break: set[str]
) -> list[str]:

    """
    Split the text strings in X on user-defined line breaks. Keep the
    left side of the split in the original row. Insert the right side
    of the split into the index slot immediately after.

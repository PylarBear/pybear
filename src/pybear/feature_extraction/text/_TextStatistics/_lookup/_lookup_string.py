...
def _lookup_string(
    ...
    When searching with string literals and `case_sensitive` is True, or
    when searching with re.Pattern objects and case is not ignored, look
    for an identical match to the given pattern. When ignoring case
    (`case_sensitive` is False for string literals or the re.Pattern
    object has an IGNORECASE flag), perform the search looking for any
    full-word matches without regard to case. If an exact match is not
    found, return an empty list. If matches are found, return a 1D list
    of the matches in their original form from the fitted data.
    ...
    _case_sensitive : Optional[bool], default = True
        Ignored if a re.compile object is passed to `pattern`. If True,

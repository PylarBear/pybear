...
# VERIFIED TEST CODE WORKS AND MLObjectOrienter PASSES 12/18/22
#
# INPUTS OF XTX, XTX_INV, TARGET, TARGET_TRANSPOSE, TARGET_AS_LIST HANDLED IN OTHER MODULES TO MANAGE COMPLEXITY
...
RAW_DATA = [[-96 ,10 ,-7] ,[14 ,-94 ,-98] ,[12 ,-96 ,-93], [47, 4, -20]]
...
MASTER_DATA_TRANSPOSE_OBJECTS = [
                                 ...
                                 sd.core_sparse_transpose(DATA_SD_TEMPLATE),
                                 None,
                                 DATA_NP_TEMPLATE.transpose(),
                                 sd.core_sparse_transpose(DATA_SD_TEMPLATE),
                                 None,
...
MASTER_XTX_RETURN_FORMAT = ['ARRAY', 'SPARSE_DICT'] #, 'AS_GIVEN']
MASTER_XTX_INV_RETURN_FORMAT = ['ARRAY', 'SPARSE_DICT'] #, 'AS_GIVEN']
# ALWAYS RETURN AN XTX(_INV) W A DATA(_TRANSPOSE) OBJECT, OR BELOW TEST CODE WILL DELETE THE DATA(_TRANSPOSE) OBJECTS BEFORE
# BUILDING THE XTX(_INV) OBJECTS BECAUSE DATA(_TRANSPOSE) IS NOT IN DUM_RETURN_OBJECTS. FIXING THIS WOULD BE A HUGE REWRITE.
MASTER_RETURN_OBJECTS = [['DATA'], ['DATA_TRANSPOSE'], ['DATA', 'DATA_TRANSPOSE'],
                         ['DATA', 'XTX'], ['DATA_TRANSPOSE', 'XTX'], ['DATA', 'XTX_INV'], ['DATA_TRANSPOSE', 'XTX_INV'],
                         ['DATA', 'XTX', 'XTX_INV'], ['DATA', 'DATA_TRANSPOSE', 'XTX', 'XTX_INV'],
                         []]

total_trials = np.product(list(map(len, [MASTER_BYPASS_VALIDATION, MASTER_DATA_OBJECTS, MASTER_DATA_RETURN_ORIENTATION,
    MASTER_DATA_RETURN_FORMAT, MASTER_DATA_TRANSPOSE_RETURN_ORIENTATION, MASTER_DATA_TRANSPOSE_RETURN_FORMAT,
    MASTER_XTX_RETURN_FORMAT, MASTER_XTX_INV_RETURN_FORMAT, MASTER_RETURN_OBJECTS])))
            ...
            for data_return_format in MASTER_DATA_RETURN_FORMAT:
                for data_transpose_return_orientation in MASTER_DATA_TRANSPOSE_RETURN_ORIENTATION:
                    for data_transpose_return_format in MASTER_DATA_TRANSPOSE_RETURN_FORMAT:
                        for xtx_return_format in MASTER_XTX_RETURN_FORMAT:
                            for xtx_inv_return_format in MASTER_XTX_INV_RETURN_FORMAT:
                                for DUM_RETURN_OBJECTS in MASTER_RETURN_OBJECTS:
                                    ...
                                    ############################################################################################################
                                        ...
                                        if exp_data_return_orientation == data_given_orientation:
                                            ...
                                            elif exp_data_return_format == 'SPARSE_DICT': EXP_DATA = sd.core_sparse_transpose(EXP_DATA)
                                        ...
                                        if 'DATA_TRANSPOSE' not in DUM_RETURN_OBJECTS:
                                            ...
                                            # 12/13/22 NOT SURE IF SHOULD HASH THESE OUT
                                            ...
                                            if exp_data_return_format == 'ARRAY' and exp_data_transpose_return_format == 'SPARSE_DICT':
                                                ...
                                                elif exp_data_return_format == 'SPARSE_DICT': EXP_DATA_TRANSPOSE = sd.core_sparse_transpose(EXP_DATA)
                                            ...
                                            if exp_data_transpose_return_orientation == exp_data_return_orientation:
                                                ...
                                                if exp_data_transpose_return_format == 'ARRAY':
                                                    ...
                                                    EXP_DATA_TRANSPOSE = sd.core_sparse_transpose(EXP_DATA_TRANSPOSE)
                                    ...
                                    exp_xtx_current_format = exp_xtx_return_format

                                    if 'XTX' or 'XTX_INV' in DUM_RETURN_OBJECTS:
                                        # IF XTX_INV NEEDED, MAKE XTX AS ARRAY AND CONVERT TO SD AFTER XTX_INV PROCESSING,
                                        # OTHERWISE RETURN XTX AS exp_xtx_return_format

                                        if not DATA is None and not DATA_TRANSPOSE is None:
                                            if data_given_orientation == 'ROW' and data_transpose_given_orientation == 'ROW':
                                                if exp_data_given_format == 'ARRAY' and exp_data_transpose_given_format == 'ARRAY':
                                                    EXP_XTX = np.matmul(DATA_TRANSPOSE, DATA)
                                                elif exp_data_given_format == 'SPARSE_DICT' and exp_data_transpose_given_format == 'SPARSE_DICT':
                                                    EXP_XTX = sd.sparse_ATA(DATA, DICT1_TRANSPOSE=DATA_TRANSPOSE,
                                                    return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)
                                                elif exp_data_given_format != exp_data_transpose_given_format:
                                                    EXP_XTX = sd.core_hybrid_matmul(DATA_TRANSPOSE, DATA,
                                                        return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)

                                            elif data_given_orientation == 'COLUMN' and data_transpose_given_orientation == 'COLUMN':
                                                if exp_data_given_format == 'ARRAY' and exp_data_transpose_given_format == 'ARRAY':
                                                    EXP_XTX = np.matmul(DATA, DATA_TRANSPOSE)
                                                elif exp_data_given_format == 'SPARSE_DICT' and exp_data_transpose_given_format == 'SPARSE_DICT':
                                                    EXP_XTX = sd.sparse_AAT(DATA, DICT1_TRANSPOSE=DATA_TRANSPOSE,
                                                        return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)
                                                elif exp_data_given_format != exp_data_transpose_given_format:
                                                    EXP_XTX = sd.core_hybrid_matmul(DATA, DATA_TRANSPOSE,
                                                        return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)

                                            elif data_given_orientation != data_transpose_given_orientation: # MEANS THEY R THE SAME THING W/ POSSIBLY DIFF FORMATS
                                                if data_given_orientation == 'ROW':
                                                    if exp_data_given_format == 'ARRAY':
                                                        EXP_XTX = np.matmul(DATA.transpose(), DATA)
                                                    elif exp_data_given_format == 'SPARSE_DICT':
                                                        EXP_XTX = sd.sparse_ATA(DATA,
                                                            return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)
                                                elif data_given_orientation == 'COLUMN':
                                                    if exp_data_given_format == 'ARRAY':
                                                        EXP_XTX = np.matmul(DATA, DATA.transpose())
                                                    elif exp_data_given_format == 'SPARSE_DICT':
                                                        EXP_XTX = sd.sparse_AAT(DATA,
                                                            return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)
                                            ...
                                            if exp_data_transpose_given_format == 'ARRAY':
                                                if data_transpose_given_orientation == 'ROW':
                                                    EXP_XTX = np.matmul(DATA_TRANSPOSE, DATA_TRANSPOSE.transpose())
                                                elif data_transpose_given_orientation == 'COLUMN':
                                                    EXP_XTX = np.matmul(DATA_TRANSPOSE.transpose(), DATA_TRANSPOSE)
                                            elif exp_data_transpose_given_format == 'SPARSE_DICT':
                                                if data_transpose_given_orientation == 'ROW':
                                                    EXP_XTX = sd.sparse_AAT(DATA_TRANSPOSE,
                                                        return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)
                                                elif data_transpose_given_orientation == 'COLUMN':
                                                    EXP_XTX = sd.sparse_ATA(DATA_TRANSPOSE,
                                                        return_as='ARRAY' if 'XTX_INV' in DUM_RETURN_OBJECTS else exp_xtx_return_format)

                                    exp_xtx_inv_given_format = None
                                    exp_xtx_inv_return_format = xtx_inv_return_format
                                    exp_xtx_inv_current_format = exp_xtx_inv_return_format

                                    if 'XTX_INV' not in DUM_RETURN_OBJECTS:
                                        EXP_XTX_INV = None
                                        exp_xtx_inv_return_format = None
                                        exp_xtx_inv_current_format = None
                                    elif 'XTX_INV' in DUM_RETURN_OBJECTS:
                                        EXP_XTX_INV = np.linalg.inv(EXP_XTX)
                                        if exp_xtx_inv_return_format == 'SPARSE_DICT':
                                            EXP_XTX_INV = sd.zip_list_as_py_float(EXP_XTX_INV)

                                    if 'XTX' not in DUM_RETURN_OBJECTS:
                                        EXP_XTX = None
                                        exp_xtx_return_format = None
                                        exp_xtx_current_format = None
                                    # IF HELD XTX AS ARRAY FOR XTX_INV, CONVERT XTX TO SD NOW IF NEEDED
                                    if exp_xtx_return_format == 'SPARSE_DICT' and isinstance(EXP_XTX, np.ndarray):
                                    ...
                                    Dummy = mloo.MLObjectOrienter(
                                        ...
                                        XTX=None,   # NOT GIVEN, ALWAYS MADE FROM DATA / DATA_TRANSPOSE FOR THIS SET OF TESTS
                                        xtx_return_format=xtx_return_format,  # XTX IS SYMMETRIC SO DONT NEED ORIENTATION

                                        XTX_INV=None,   # NOT GIVEN, ALWAYS MADE FROM DATA / DATA_TRANSPOSE FOR THIS SET OF TESTS

...
def _val_refit(
    ...
    ----------
    _refit:
        Union[None, bool, str, callable], default=True - Whether to
        refit the estimator on the 'best' parameters after completing
        the grid search, and if so, which scoring metric to use to
        determine the 'best' parameters.
    _scorer:
        dict[str, callable] - previously validated scorer object, must
        be dict[str, callable]. used to determine the number of scorers,
        which impacts what values are allowed for the refit param.

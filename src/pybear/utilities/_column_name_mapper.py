...
def column_name_mapper(
    ...
    try:
        ...
        if isinstance(feature_names_in, (str, dict)):
            raise Exception
        feature_names_in = np.array(list(feature_names_in))
        if len(feature_names_in.shape) != 1:
            raise Exception
        if len(feature_names_in) == 0:
            raise Exception
        if not all(map(
            isinstance,
            feature_names_in.tolist(),
    ...
    if all(map(isinstance, feature_names, (str for _ in feature_names))):
        ...
        if np.any(nan_mask_numerical(feature_names)):
            raise TypeError(err_msg)
    ...
    if is_str:
        ...
        for idx, _column in enumerate(feature_names):
            ...
            MASK = (feature_names_in == _column)
            feature_names[idx] = int(np.arange(_n_features_in)[MASK][0])
    ...
    if feature_names_in is None:
        # without knowing feature_names_in, all we have is a feature_names
        ...
        return np.array(list(feature_names), dtype=np.int32)
    elif feature_names_in is not None:  # must be 1D ndarray of strings, len >= 1
        _n_features_in = len(feature_names_in)
        if min(feature_names) < -_n_features_in:
            raise ValueError(
                f"column index {min(feature_names)} is out of bounds for a "
                f"feature name vector with {_n_features_in} features"
            )
        if max(feature_names) >= _n_features_in:
            raise ValueError(
                f"column index {max(feature_names)} is out of bounds for a "
                f"feature name vector with {_n_features_in} features"
            )
        ...
        if positive is None:
            ...
            feature_names = [
                x + (_n_features_in if x < 0 else 0) for x in feature_names
            ]
        elif positive is False:
            # make sure all are negative
            feature_names = [
                x - (_n_features_in if x > -1 else 0) for x in feature_names
            ]

...
def manual_num_seq_list_fill(name, VAR_LIST, size, min=float('-inf'), max=float('inf')):
    ...
    while iteration < __:
        if iteration > 0:
            if var == 'F':  # IF USER ENTERED THIS ON LAST GO-AROUND, LOOP UNTIL LIST IS FULL
        ...
        while True:
            try:
                ...
                if var in 'EF': break
                else: print(f'Must be > {min} and < {max}, "f" after first iteration, or "e"')

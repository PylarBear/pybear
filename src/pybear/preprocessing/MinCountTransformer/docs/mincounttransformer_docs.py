...
def mincounttransformer_docs():
    ...
    delete_axis_0 : bool, default=False
        ...
        however, the default behavior is used and examples associated with
        the minority value are removed and the feature is then also
    ...
    numpy arrays, pandas dataframes, and pandas series. MCT does not require
    that y be passed to any method, and simply not passing a y argument
    circumvents these constraints. The dask_ml wrappers (currently) do
    not accept a y argument to :meth: transform, and MCT is bound to this
    condition. However, the need to mask y identically to the masking of
    X still exists. A workaround is to :term: fit on X only, use the
    get_row_support method of MCT to get the mask generated for X, and
    apply the mask to y separately.

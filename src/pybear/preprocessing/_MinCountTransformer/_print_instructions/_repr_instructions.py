...
def _repr_instructions(
    ...
    _delete_instr:
        ...
        columns as generated by _make_instructions() from the
        uniques / counts in _total_counts_by_column and the parameter
        settings.
    _total_counts_by_column:
        TotalCountsByColumnType - the uniques and their counts for each
        column in the data.
    _thresholds:
        CountThresholdsType - the threshold value(s) that determine
        whether a unique value is removed from a dataset.
    _n_features_in:
        int - the number of features in the data.
    _feature_names_in:
        Union[FeatureNamesInType, None] - the features names of the data
        if the data was passed in a container that had features names,
        like a pandas or polars dataframe. Otherwise, None.
    _clean_printout:
        bool - Truncate printout to fit on screen.
    _max_char:
        Optional[numbers.Integral], default=99 - the maximum number
        of characters to display per line if :param: 'clean_printout'
        is set to True. Ignored if 'clean_printout' is False. Must be
        an integer in range [72, 120].

...
for _cv in CV:
    for _scoring in SCORING:
        for return_train in RETURN_TRAIN:
            ctr += 1
            print(f'\033[92mRunning test permutation number {ctr} of'
                  f' {test_permutations}...\033[0m')
            ...
            for sub_scoring in _scoring:
                COLUMN_CHECK += ['best_threshold' +
                    ('' if len(_scoring) == 1 else f'_{sub_scoring}')
                ]
            ...
            for out_col in OUTPUT_COLUMNS:
                if out_col not in COLUMN_CHECK:
                    ...
                    print(COLUMN_CHECK)
                    raise Exception(
                        f"\033[91m{out_col} is in OUTPUT_COLUMNS but not in "
                        f"COLUMN_CHECK\033[0m"
                    )

            for check_col in COLUMN_CHECK:
                if check_col not in OUTPUT_COLUMNS:
                    raise Exception(
                        f"\033[91m{check_col} is in COLUMN_CHECK but not "
                        f"in OUTPUT_COLUMNS\033[0m"
                    )

            if OUTPUT_LEN != correct_cv_results_len:
                raise Exception(
                    f"\033[91moutput rows ({output_len}) does not equal "
                    f"expected rows ({correct_cv_results_len})\033[0m"
                )

...
class TestDaskAttrsPostFit:
    ...
    def test_dask(self, _refit, _format, _scoring, param_grid_dask_log,
        dask_est_log, standard_cv_int, _refit_false, generic_no_attribute_1,
        ...
        assert getattr(_dask_GSTCV, 'n_splits_') == standard_cv_int
            ...
            if len(_dask_GSTCV.scorer_) == 1 or \
                ...
                if attr == 'best_params_':
                    ...
                    for param, best_value in __.items():
                        ...
                        assert param in param_grid_dask_log
                        # best value was in grid
                        assert best_value in param_grid_dask_log[param]
        ...
        # instances, or dicts that can use 'isinstance' or '==' ** * ** *
            ...
            assert isinstance(__, type(dask_est_log))
            assert check_is_fitted(__) is None  # is fitted; otherwise cif() raises

            og_params = _dask_GSTCV.estimator.get_params(deep=True)
            ...
            for param, value in __.get_params(deep=True).items():
                ...
                # passed estimator
                if param not in _dask_GSTCV.best_params_:
                    if value is np.nan:
                ...
                elif param in _dask_GSTCV.best_params_:
                    if value is np.nan:
                        assert _dask_GSTCV.best_params_[param] is np.nan
                    else:
                        assert value == _dask_GSTCV.best_params_[param]

            __ = getattr(_dask_GSTCV, 'refit_time_')
            assert isinstance(__, float)
            assert __ > 0

            assert getattr(_dask_GSTCV, 'n_features_in_') == _cols

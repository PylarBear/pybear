...
class TestCoreFitPipelineAccuracy:
    ...
    @staticmethod
    @pytest.fixture
    def special_sk_pipe_log(sk_standard_scaler, special_sk_est_log):
        return Pipeline(
            steps=[
                ('sk_StandardScaler', sk_standard_scaler),
                ('sk_logistic', special_sk_est_log)
            ]
        )
    ...
    @staticmethod
    @pytest.fixture()
    def special_sk_GSCV_pipe_log_one_scorer_prefit(sk_gscv_init_params,
        special_sk_pipe_log, param_grid_pipe_sk_log
    ):

        __ = sk_GSCV(**sk_gscv_init_params)
        __.set_params(
            estimator=special_sk_pipe_log,
        ...
        (
            [
                {'sk_StandardScaler__with_mean': [True, False],
                 'sk_logistic__C': [.001, .01]}
            ],
            [
                {'sk_logistic__C': [.0001, .001]},
                {'sk_StandardScaler__with_mean': [True, False],
                 'sk_logistic__C': [.001, .01]}
            ],
        )
    ...
    def test_accuracy_vs_sk_gscv(self, _param_grid, standard_cv_int,
        standard_error_score, _scorer,  _n_jobs, _return_train_score, X_np,
        y_np, special_sk_pipe_log, special_sk_GSCV_pipe_log_one_scorer_prefit):
        ...
        out_sk_gscv = special_sk_GSCV_pipe_log_one_scorer_prefit

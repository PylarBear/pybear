...
from pybear.preprocessing.NoDupPolyFeatures._validation._degree__min_degree import \
...
class TestDegree__MinDegree:
    ...
    @pytest.mark.parametrize('_degree', junk + bad + good)
    @pytest.mark.parametrize('_min_degree', junk + bad + good)
    def test_validation(self, _degree, _min_degree):
        ...
        if isinstance(_degree, bool) or \
            ...
            _degree not in range(100) or \
            _min_degree not in range(100) or \
            _degree < _min_degree:

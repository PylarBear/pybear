...
from pybear.preprocessing.InterceptManager._shared._manage_keep import _manage_keep
...
class TestManageKeep:
    ...
    #     Callable[[DataFormatType], int]              ---- converted to int, validated against constant_columns_
    #     Literal['first', 'last', 'random', 'none'],  ---- passes thru, not validated
    #     str,                                         ---- converted to int via feature_names_in_, validated against constant_columns_
    #     int,                                         ---- validated against constant_columns_
    # ]]



    # callable keep converts X to int, validated against constant_columns_
    # words='keep' callable has returned an integer column index"

    # str keep in header converted to int, validated against constant_columns_
    # words=f"'keep' was passed as feature name corresponding to column index"

    # int keep validated against constant_columns_
    # words=f"'keep' was passed as column index"

    # keep in ('first', 'last', 'random', 'none')
    # converts to int

    # isinstance(_keep, dict)
    # just passes thru
    ...
    # dict v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^
    ...
    def test_dict_passes_thru(self, _X_np, _X_pd, _format, _keep, _constant_columns, _shape):
        ...
        out = _manage_keep(
            _keep=_keep,
            _X=_X,
            _constant_columns=_constant_columns,
        ...
        assert out == _keep, \
            f"_manage_keep altered keep dict[str, any]"
    # END dict v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^
    ...
    @pytest.mark.parametrize('_constant_columns', ({}, {0:1, 1:np.nan, 2:0}))
    def test_callable(self, _X_np, _X_pd, _format, _keep, _constant_columns, _shape):
        ...
        if _keep(_X) in _constant_columns:
            ...
            assert isinstance(out, int), f"_manage_keep callable did not return integer"
            assert out == _keep(_X), \
    ...
    # END callable v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^

    # feature name str v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^
    @pytest.mark.parametrize('_keep', ('column_1', 'column_2', 'column_3'))
    @pytest.mark.parametrize('_constant_columns', ({}, {0: 1, 1: np.nan}))
    def test_feature_name_str(self, _X_pd, _keep, _columns, _constant_columns, _shape):

        # _keep_and_columns caught if keep feature name:
        # - was passed with no header (ie for a numpy array) - dont need to test np, impossible condition.
        ...
        if exp in _constant_columns:
            ...
            assert isinstance(out, int), f"_manage_keep feature name did not return integer"
            assert out == exp, \
    ...
    # END feature name str v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v


    # literal str v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^
    @pytest.mark.parametrize('_format', ('np', 'pd'))
    ...
    def test_literal_str(self, _X_np, _X_pd, _columns, _format, _keep, _constant_columns, _shape):

        # what we know:
        # the only possible strings that can get to _manage_keep besides
        # feature names are literals ('first', 'last', 'random', 'none') case-sensitive!
        # any other string would except in _keep_and_columns
        # only need to test the exact cases of the literals
        # unless 'keep' is 'none' or constant_columns_ is empty, the returned value
        # must be in constant_columns_. if constant_columns_ is empty, returns 'none'
        #
        ...
        if _keep in ('first', 'last', 'none') or len(_constant_columns) == 0:
            assert out == exp, \
                (f"_manage_keep did not return expected keep literal index exp {exp}, got {out}")
        elif _keep == 'random':
            assert out in range(_shape[1])
    # END literal str v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^
    ...
    def test_integer(self, _X_np, _X_pd, _format, _keep, _constant_columns, _shape):

        # from _keep_and_columns, we already know that keep integer is in
        # range(X.shape[1]. just needs to verify the idx is actually a column of constants.
        ...
        if _keep in _constant_columns:
            ...
            assert isinstance(out, int), f"_manage_keep integer did not return integer"
            assert out == _keep, \
                (f"_manage_keep integer did not return expected integer. exp {_keep}, got {out}")
    ...
    # END int v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^v^

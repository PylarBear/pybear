...
class TestDaskIncrementalParallelPostFit:
    ...
    @pytest.fixture
    def X():
        return np.random.randint(0, 10, (200,20))


    @staticmethod
    @pytest.fixture
    def y():
        return np.random.randint(0, 2, (200,2))

    @staticmethod
    @pytest.fixture
    def COLUMNS():
        return [str(uuid4())[:5] for _ in range(20)]
    ...
    def test_always_fits_X_y_always_excepts_transform_with_y(self, wrappings,
        ...
        if wrappings in ['incr', 'both'] and (a + b) == 1:

            # this is a dask_ml raised Exception, let it raise whatever

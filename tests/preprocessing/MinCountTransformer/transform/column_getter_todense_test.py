    ...
    def test_accuracy(self, _has_nan, _dtype, _format, _col_idx1, _shape):
        ...
        if isinstance(_X_wip,
            ...
            with pytest.raises(AssertionError):
                _column_getter_to_dense(_X_wip, _col_idx1)
            pytest.skip(reason=f"cant do more tests after exception")
        else:
            column1 = _column_getter_to_dense(_X_wip, _col_idx1)

        assert len(column1.shape) == 1

        # all objects return the column as a dense np 1D vector. straightforward
        # to just compare _column_getter_to_dense against the original _X to
        # ensure the correct column is being pulled
        if isinstance(_X, np.ndarray):
            og_col = _X[:, _col_idx1]
        elif isinstance(_X, pd.core.frame.DataFrame):
            og_col = _X.iloc[:, _col_idx1]
        else:
            raise Exception

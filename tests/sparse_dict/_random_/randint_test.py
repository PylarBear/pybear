...
class TestRandintValidation:
    ...
    def test_rejects_junk_max(self, _max):
        with pytest.raises(TypeError):
            randint(-100, _max, (3,3), 50, dtype=np.int8)
    ...
    def test_rejects_junk_shape(self, _shape):
        with pytest.raises(TypeError):
            randint(-5, 5, _shape, 50, dtype=np.int8)
    ...
    def test_rejects_junk_sparsity(self, _sparsity):
        with pytest.raises(TypeError):
            randint(-10, 10, (3,3), _sparsity, _dtype=np.int8)
    ...
    def test_rejects_junk_dtype(self, junk_dtype):
        with pytest.raises(TypeError):
            randint(0, 3, (3,3), 50, dtype=junk_dtype)

    @pytest.mark.parametrize('float_dtype',
        (np.float16, np.float32, np.float64, float)
    )
    def test_rejects_float_dtype(self, float_dtype):
        with pytest.raises(TypeError):
            randint(0, 5, (3,3), 50, dtype=float_dtype)
    ...
    def test_accepts_good_dtype(self, good_dtype):
        randint(0, 5, (3,3), 50, dtype=good_dtype)
...
@pytest.mark.parametrize('_dtype', (int, np.int8, np.int16, np.int32,
                        np.int64, np.uint8, np.uint16, np.uint32, np.uint64)
)
class TestRandintAccuracy:
    ...
    def sparse_dict_output(self, _min, _max, _shape, _sparsity, _dtype):
        ...
        sd = randint(_min, _max, _shape, _sparsity, _dtype)
        return sd

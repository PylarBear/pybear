...
class TestCreateRandomSparseValidation:
    ...
    def test_rejects_junk_min(self, _min):
        with pytest.raises(TypeError):
            _create_random_sparse_dict(_min, 10_000, (3,3), 50, dtype=np.int8)

    @pytest.mark.parametrize('_min', (float('-inf'), float('inf'), np.nan))
    def test_rejects_bad_min(self, _min):
        with pytest.raises(ValueError):
            _create_random_sparse_dict(_min, 100, (3,3), 50, dtype=np.int8)
    ...
    def test_rejects_junk_max(self, _max):
        with pytest.raises(TypeError):
            _create_random_sparse_dict(-100, _max, (3,3), 50, dtype=np.int8)
    ...
    def test_rejects_junk_shape(self, _shape):
        with pytest.raises(TypeError):
            _create_random_sparse_dict(-5, 5, _shape, 50, dtype=np.int8)
    ...
    def test_rejects_junk_sparsity(self, _sparsity):
        with pytest.raises(TypeError):
            _create_random_sparse_dict(-10, 10, (3,3), _sparsity, _dtype=np.int8)
    ...
    def test_rejects_bad_sparsity(self, _sparsity):
        with pytest.raises(ValueError):
            _create_random_sparse_dict(-10, 10, (3,3), _sparsity, dtype=np.int8)

    @pytest.mark.parametrize('_sparsity', (0, 50, 100))
    def test_accept_good_sparsity(self, _sparsity):
        _create_random_sparse_dict(0, 10, (3,3), _sparsity, dtype=np.int8)

    @pytest.mark.parametrize('junk_dtype',
        (0, 'junk', np.pi, True, None, [], {}, (), lambda x: x)
    )
    def test_rejects_junk_dtype(self, junk_dtype):
        with pytest.raises(TypeError):
            _create_random_sparse_dict(0, 3, (3,3), 50, dtype=junk_dtype)
    ...
    def test_accepts_good_dtype(self, good_dtype):
        _create_random_sparse_dict(0, 5, (3,3), 50, dtype=good_dtype)
...
@pytest.mark.parametrize('_dtype', (int, float, np.int8, np.int16, np.int32,
    np.int64, np.uint8, np.uint16, np.uint32, np.uint64, np.float16,np.float32,
    np.float64)
)
class TestCreateRandomSparseAccuracy:
    ...
    def invalid_test_case(self, _min, _dtype):
        return (_min < 0 and _dtype in (np.uint8, np.uint16, np.uint32, np.uint64))


    @pytest.fixture
    def sparse_dict_output(self, _min, _max, _shape, _sparsity, _dtype):

        if self.invalid_test_case(_min, _dtype):
            pytest.skip(f"parameter combination is invalid")

        sd = _create_random_sparse_dict(_min, _max, _shape, _sparsity, _dtype)
        return sd
    ...
    def test_min_max(self, sparse_dict_output, _min, _max,
                     _shape, _sparsity, _dtype):

        if 0 in _shape:
            pytest.skip(reason=f'no values to get min/max of if empty')

        # get exp min / max ** * ** * ** * ** * ** * ** * ** * ** * ** * ** *
        exp_value_dtype = str(_dtype)

        if 'int' not in exp_value_dtype and 'float' not in exp_value_dtype:
            raise TypeError(f'invalid type "{type(_dtype)}"')

        if _sparsity < 0 or _sparsity > 100:
            raise ValueError(f'sparsity ({_sparsity}) is out of bounds')

        if 'int' in exp_value_dtype:
        ...
        if _sparsity == 0:
            if 'int' in exp_value_dtype:
                exp_min = exp_min if exp_min != 0 else 1
                exp_max = exp_max if exp_max != 0 else -1
            elif 'float' in exp_value_dtype:
                # exp_min = exp_min
                # exp_max = exp_max
                pass
    ...
    def test_value_dtype(self, sparse_dict_output, _shape, _dtype):
        ...
        assert VALUE_DTYPES[0] == str(_dtype), f"act_value_dtype != exp_value_dtype"

...
from pybear.base._check_2D_num_array import check_2D_num_array
...
class TestCheck2DNumArray:
    ...
    def test_rejects_1D(self):
        ...
        with pytest.raises(TypeError):
            check_2D_num_array(list(_base_np))

        with pytest.raises(TypeError):
            check_2D_num_array(tuple(list(_base_np)))

        with pytest.raises(TypeError):
            check_2D_num_array(_base_np)

        with pytest.raises(TypeError):
            check_2D_num_array(pd.Series(_base_np))

        with pytest.raises(TypeError):
            check_2D_num_array(pl.Series(_base_np))


    @pytest.mark.parametrize('require_finite', (True, False))
    @pytest.mark.parametrize('has_non_finite', (True, False))
    @pytest.mark.parametrize('container',
        (list, tuple, np.array, ss.csr_array, ss.dok_array, ss.lil_array,
         ss.bsr_array, pd.DataFrame, pl.DataFrame)
    )
    ...
    def test_accuracy(
        ...
        _base_np = np.random.uniform(0, 1, _shape)

        if has_non_finite:
            _pool = (np.nan, np.inf, 'nan', float('-inf'))

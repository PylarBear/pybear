...
from pybear.feature_extraction.text._TextRemover._validation._str_remove \
    import _val_str_remove



class TestValStrRemove:


    @pytest.mark.parametrize('junk_single',
        (-2.7, -1, 0, 1, 2.7, True, False, {'A': 1}, lambda x: x)
    )
    def test_rejects_junk_single(self, junk_single):
        # could be None, str, set[str], list[of the 4]
        with pytest.raises(TypeError):
            _val_str_remove(junk_single, list('abcde'))


    @pytest.mark.parametrize('good_single_sep', (None, ', ', {' ', ', ', '. '}))
    def test_accepts_good_single(self, good_single_sep):
        # could be None, str, set[str]
        _val_str_remove(good_single_sep, list('abcde'))
    ...
    def test_rejects_junk_sep_as_seq(self, junk_container, junk_seps):
        ...
        with pytest.raises(TypeError):
            _val_str_remove(junk_sep, list('ab'))
    ...
    def test_rejects_bad_sep_as_seq(self):
        ...
        with pytest.raises(ValueError):
            _val_str_remove([str(uuid.uuid4())[:4] for _ in range(6)], list('abcde'))


        # too short
        with pytest.raises(ValueError):
            _val_str_remove([str(uuid.uuid4())[:4] for _ in range(4)], list('abcde'))

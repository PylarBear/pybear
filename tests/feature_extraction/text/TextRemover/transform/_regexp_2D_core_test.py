...
from pybear.feature_extraction.text._TextRemover._transform._regexp_2D_core \
...
class TestRegExp2DCore:
    ...
    def test_accuracy(self, _text):
        ...
        # -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
        # one removed
        out_X, out_mask = _regexp_2D_core(
            deepcopy(_text),
            re.compile('a'),
            None
        )
        ...
        out_X, out_mask = _regexp_2D_core(
            deepcopy(_text),
            ['a|b', False, False, False, re.compile('z')],
            None
        )
        ...
        out_X, out_mask = _regexp_2D_core(
            deepcopy(_text),
            [re.compile('a|b'), False, False, False, 'z'],

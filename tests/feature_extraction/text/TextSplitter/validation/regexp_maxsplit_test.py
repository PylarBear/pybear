...
from pybear.feature_extraction.text._TextSplitter._validation._regexp_maxsplit \
    import _val_regexp_maxsplit
...
class TestValStrMaxSplit:
    ...
    def test_rejects_junk_single_sms(self, junk_single_sms):
        ...
        with pytest.raises(TypeError):
            _val_regexp_maxsplit(junk_single_sms, list('abcde'))
    ...
    def test_rejects_junk_sms_sequence(self, junk_ms):
        ...
        with pytest.raises(TypeError):
            _val_regexp_maxsplit(junk_ms, list('ab'))
    ...
    def test_rejects_bad_sms_len(self, bad_ms_len):
        ...
        with pytest.raises(ValueError):
            _val_regexp_maxsplit(list(range(1,9))[:bad_ms_len], list('abcde'))


    def test_accepts_single_None_single_int(self):

        _val_regexp_maxsplit(None, list('abcde'))

        _val_regexp_maxsplit(-10_000, list('abcde'))

        _val_regexp_maxsplit(0, list('abcde'))
    ...
    def test_accepts_good_sequence(self):
        ...
        for trial in range(20):
            ...
            _val_regexp_maxsplit(
                _regexp_maxsplit=_maxsplit,
                _X = list('abcde')
            )

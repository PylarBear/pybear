...
from pybear.feature_extraction.text._TextSplitter._validation._regexp_sep import \
    _val_regexp_sep




class TestValRegExpSep:


    @pytest.mark.parametrize('junk_single_sep',
        (-2.7, -1, 0, 1, 2.7, True, False, {'A': 1}, lambda x: x)
    )
    def test_rejects_junk_regexp_as_single(self, junk_single_sep):
        # could be None, str, or re.Pattern
        with pytest.raises(TypeError):
            _val_regexp_sep(junk_single_sep, list('abcde'))
    ...
    def test_accepts_good_single(self, good_single_regexp):
        ...
        _val_regexp_sep(good_single_regexp, list('abcde'))
    ...
    def test_rejects_junk_regexp_as_seq(self, junk_seq_regexp):
        ...
        with pytest.raises(TypeError):
            _val_regexp_sep(junk_seq_regexp, list('ab'))
    ...
    def test_rejects_bad_regexp_as_seq(self):
        ...
        with pytest.raises(ValueError):
            _val_regexp_sep(['\W' for _ in range(6)], list('abcde'))


        # too short
        with pytest.raises(ValueError):
            _val_regexp_sep(['\W' for _ in range(4)], list('abcde'))
